...
TOOL_NAME = "SENTRYLOG"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'SL-T1059-001', 'severity': 'high', 'weight': 2.5, 'title': 'POWERSHELL_ENCODED_CMD', 'when': {'event_type': 'process_create', 'command_line': '(?i)powershell.*-enc'}, 'remediation': 'Investigate encoded PowerShell — common Cobalt Strike loader pattern.'}, {'id': 'SL-T1078-001', 'severity': 'high', 'weight': 2.5, 'title': 'AUTH_FAIL_BURST', 'when': {'event_type': 'auth_fail', 'count': '^[5-9]\\d*|^[1-9]\\d{2,}'}, 'remediation': 'Likely brute-force. Enforce MFA and lockout.'}, {'id': 'SL-T1486-001', 'severity': 'critical', 'weight': 3.0, 'title': 'RANSOMWARE_EXT', 'when': {'event_type': 'file_rename', 'new_name': '(?i)\\.(locked|encrypted|crypt|wcry|locky)$'}, 'remediation': 'Isolate host. Activate IR plan.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="detection",
                    metadata=item,
                ))

"""Native cognis-connect emit for sentrylog — forward findings to any platform.

Maps sentrylog's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    sentrylog ... --format json | sentrylog-emit --to stix
    sentrylog-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['description'] = rec.get('data', {}).get('description', '')
        out['tags'] = rec.get('tags', [])
        out['ipv4'] = rec.get('data', {}).get('ipv4')
        out['domain'] = rec.get('data', {}).get('domain')
        out['url'] = rec.get('data', {}).get('url')
        out['sha256'] = rec.get('data', {}).get('sha256')
        out['cve'] = rec.get('data', {}).get('cve')
        out['imo'] = rec.get('data', {}).get('imo')
        out['mmsi'] = rec.get('data', {}).get('mmsi')
        out['lat'] = rec.get('data', {}).get('lat')
        out['lon'] = rec.get('data', {}).get('lon')
        return out
    except Exception:
        return rec
